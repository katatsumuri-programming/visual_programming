...
//   var goog = window.Blockly.goog;
  ...
  funcs.Program = funcs.BlockStatesment = (node, st, c) => {
    for (let i = 0; i < node.body.length; ++i)
      c(node.body[i], st, "Statement")
  }
  ...
  funcs.SwitchStatement = (node, st, c) => {
    ...
    var mutation1 = newNode('mutation', {items:node.cases.length-hasDefault}) // TODO: Take out +1 when items count corrected on AddSub block
    block1.appendChild(mutation1)
  ...
  funcs.ObjectExpression = (node, st, c) => {
    ...
    var node1 = current_node;
    for (let i = 0; i < node.properties.length; ++i){
      let property1 = newNode('value',{name:'items'+i});
  ...
  funcs.CallExpression = (node, st, c) => {
    ...
    if(node.callee.type === "MemberExpression"){
      ...
      // if (node.callee.name == "output") {
        // block1 = newNode('block', {type:'output'}, '', node);
        // var outputValue = newNode('value',{name:'output'},'');
        // var textBlock = newNode('block', {type:'text'}, '', node);
        // textBlock.appendChild(newNode('field',{name:'TEXT'},node.arguments[0].value, name));
        // outputValue.appendChild(textBlock);
        // block1.appendChild(outputValue);
      // } else {
        if(expression_statement){
          expression_statement = false;
