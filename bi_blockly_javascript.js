...
Blockly.JavaScript['bi_switch'] = function(block) {
  ...
  var codeArrCase = new Array(block.itemCount_);
  var codeArrDO = new Array(block.itemCount_);
  var caseCode = "";
  if (!(block.itemCount_ == 0)) {
    for (var n = 0; n < block.itemCount_; n++) {
      codeArrCase[n] = Blockly.JavaScript.valueToCode(block, 'items' + n, Blockly.JavaScript.ORDER_COMMA);
    }
  }
...
Blockly.JavaScript['bi_call_return'] = function(block) {
  // Create a list with any number of elements of any type.
  var text_name = block.getFieldValue('NAME');
  var value_chain = Blockly.JavaScript.valueToCode(block, 'chain', Blockly.JavaScript.ORDER_ATOMIC);
  var codeArr = new Array(block.itemCount_);
